        ...
        if (index != 0)
        {
            ...
            if (new_index + 1 < (m_main_tab_widget->count() - 1))
            {
                ...
                for (uint16_t ind = index; ind < m_kia_settings->m_kia_gui_settings->m_data_tabs.size(); ++ind)
                {
                    ...
                    {
                        m_kia_settings->m_kias_view_data->m_data_graph[el.toInt()][QP_NUM_MAIN_TAB_WIDGET] = QString::number(m_kia_settings->m_kias_view_data->m_data_graph[el.toInt()][QP_NUM_MAIN_TAB_WIDGET].toInt() - 1);
                    }
                    for (auto el : m_kia_settings->m_kias_view_data->m_data_tables_on_tabs[ind + 1])
                    {
                        m_kia_settings->m_kias_view_data->m_data_table[el.toInt()][QP_NUM_MAIN_TAB_WIDGET] = QString::number(m_kia_settings->m_kias_view_data->m_data_table[el.toInt()][QP_NUM_MAIN_TAB_WIDGET].toInt() - 1);
                    }
                }
            }
